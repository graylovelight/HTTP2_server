...
def send_response(h2_conn, stream_id, headers):
    ...
    for i in range(0, len(response_data), max_frame_size):
        end_stream = (i + max_frame_size) >= len(response_data)
        h2_conn.send_data(stream_id, response_data[i:i + max_frame_size], end_stream=end_stream)
